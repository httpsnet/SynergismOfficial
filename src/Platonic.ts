...
export const createPlatonicDescription = (index: number) => {
    ...
    DOMCacheGetOrSet('platonicHepteractCost').textContent = format(player.hepteractCrafts.abyss.BAL) + '/' + format(Math.floor(platUpgradeBaseCosts[index].abyssals * priceMultiplier), 0, platUpgradeBaseCosts[index].abyssals * priceMultiplier < 1e10) + ' Hepteracts of the Abyss'
