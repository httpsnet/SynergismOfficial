...
export const revealStuff = () => {
    ...
    for (const ex of Array.from(example32)) { // Ability to use and gain hepteracts //
        ex.style.display = player.unlocks.hepteract ? 'block' : 'none';
    }
    ...
    player.highestSingularityCount >= 25 ? //Auto Fragments Buy (After Cx1)
    ...
    player.highestSingularityCount > 0 && player.unlocks.hepteract ?
    ...
    const bbshardUnlocks = document.getElementsByClassName('bbshards') as HTMLCollectionOf<HTMLElement>;
    for (const item of Array.from(bbshardUnlocks)) { // Stuff that you need octeracts to access
        item.style.display = player.singularityUpgrades.ultimatePen.getEffect().bonus ? 'block' : 'none';
    }
    ...
    const automationUnlocks: Record<string, boolean> = {
        ...
        'automationsAutoUpgrade': player.highestSingularityCount >= 25, // Feature - Upgrades - Auto Buy Automation Upgrades
        ...
        'toggle35': player.unlocks.hepteract && player.highestSingularityCount > 0, // Hepteracts - Notifications
